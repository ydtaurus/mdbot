...
vipro = LINETCR.LINE()
#vipro.login(qr=True)
vipro.login(token='Eqg3Kad2rZetAYLp31Z8.Pm3tmhivOE+zKhr8Q+SPEa.P+4Ku2/0IeaJOPDgkx4oUfHtB8C79f037lyFhGLoRBY=')
vipro.loginResult()
...
╚═════════════════════════
"""
#tjia="u33caed56a18c666015b059e325f2704d"
...
Bots=[mid]
Creator= "u33caed56a18c666015b059e325f2704d" #bukan mid bot nya ya tapi mid si pembuat bot
# admin= "u33caed56a18c666015b059e325f2704d" #yang ini samakan aja kayak mid creator
admin= "ua4cdd3307215a1efd990747cabfb2978" #yang ini samakan aja kayak mid creator
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"Bot Auto Like ©By : MasterDiceBot\nContact My Creator : 👉 line.me/ti/p/~ydtaurus",    
    "commentOn":True,
    "commentBlack":{},
...
#                            Name = summon(op.param2)
                            if Name in cctv['sidermem'][op.param1]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        vipro.sendText(op.param1, "Haii " + "☞ " + Name + " ☜" + "\nNgintip Aja Lu. . .\nUeue Bareng kita Sini (^__-)   ")
                                        time.sleep(0.2)
                                        summon(op.param1,[op.param2])
                                    else:
                                        vipro.sendText(op.param1, "Haii " + "☞ " + Name + " ☜" + "\nBetah Banget Jadi Sider. . .\nGa Punya Idung Lu?\nGabung Chat Napa (-__-)   ")
                                        time.sleep(0.2)
                                        summon(op.param1,[op.param2])
                                else:
                                    vipro.sendText(op.param1, "Haii " + "☞ " + Name + " ☜" + "\nNgapain Lu Jadi Sider???\nSini Gabung Chat...   ")
                                    time.sleep(0.2)
                                    summon(op.param1,[op.param2])
                        else:
                            pass
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = vipro.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["Ngapain ngeTag Gw Asw!",cName + "Ngapain Lu Ngetag? Lo Siapa Njir?",cName + "Ngetag Gw? Ngajakin Ueue?","Bot Lagi Coli\nJangan Dicari", cName + "Kenapa Tag Gw Asw?","Dilan Cakhep Lagi Tidur\nJangan Di Tag " + cName, "Jangan Tag Gua Mulu Lu Vangsat " + cName, "Lu Siapa " + cName + "?", "Ada Perlu Apa " + cName + "?", cName + "Ngetag Gw... Sok Kenal Lu!", cName + "Kurang Kerjaan Nih Si JONES\nNgetag BOT Mulu", cName + "Lu Dicariin Dijjah Yellow Tuh", cName + "MimiPeri Kirim Salam. . .","Woii " + cName + " Jangan Ngetag, Ueue Kita?!"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
		...
		if msg.from_ in Creator:
		    for i in gid:
			vipro.sendText(i,"=======[BROADCAST]=======\n\n"+bc+"\n\nContact Me : line.me/ti/p/~ydtaurus.")
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                link = ["https://profile.line-scdn.net/0hPAKp-WRkD252CSOCzVpwOUpMAQMBJwkmDm1CWgNdBFpcaUgxGGhHD1IJVQ1aPUBsSGxEX1APUFsO"]
